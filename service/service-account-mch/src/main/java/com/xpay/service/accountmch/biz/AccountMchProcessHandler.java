 ...
 */
@Component
public class AccountMchProcessHandler {
    private Logger logger = LoggerFactory.getLogger(AccountMchProcessHandler.class);
    //每次账务处理允许的商户数
    ...
    public boolean executeSync(AccountMchRequestDto requestDto, AccountMchProcessDto processDto) {
        List<AccountMchProcessDto> processVoList = new ArrayList<>(1);
    ...
    public boolean executeSync(AccountMchRequestDto requestDto, List<AccountMchProcessDto> processDtoList) {
        ...
        accountMchProcessBiz.findDebitDetailAndFilterRepeatVo(processDtoList);
        if (processDtoList.isEmpty()) {
            logger.info("accountProcessNo={} 全部均已经账务处理过，无需重复处理！", accountProcessNo);
            throw AccountMchExceptions.ACCOUNT_PROCESS_REPEAT.newWithErrMsg("请勿重复账务处理");
        }

        //3、按用户分组
        Map<String, List<AccountMchProcessDto>> groupMap = processDtoList.stream().collect(Collectors.groupingBy(AccountMchProcessDto::getMerchantNo));

        //4.账务处理前置处理
        try {
            accountMchProcessBiz.beforeSyncExecute(requestDto, processDtoList, false);
        } catch (Throwable e) {
        ...
        try {
            logger.info("accountProcessNo={} 商户账户获取锁开始", accountProcessNo);
            lockList = redisClient.tryLock(LockNameUtil.getAccountMchLockName(groupMap.keySet()), DistLockConst.ACCOUNT_LOCK_WAIT_MILLS, DistLockConst.ACCOUNT_LOCK_EXPIRE_MILLS);
            logger.info("accountProcessNo={} 商户账户获取锁结束 lockList.size={}", accountProcessNo, lockList.size());
        } catch (Throwable t) {
            logger.error("accountProcessNo={} groupMap.size={} 商户账户获取锁出现异常", accountProcessNo, groupMap.size(), t);
        } finally {
            if (lockList == null || lockList.size() != groupMap.size()) {
                logger.error("accountProcessNo={} groupMap.size={} lockList.size={} 商户账户获取锁失败", accountProcessNo, groupMap.size(), lockList == null ? 0 : lockList.size());
                try {
                    //回退到'待处理'
                    accountMchProcessBiz.beforeSyncExecute(requestDto, processDtoList, true);
                } catch (Throwable e) {
                    logger.error("accountProcessNo={} 平台商户回退到'待处理'时发生异常", accountProcessNo, e);
                    throwBizExceptionIfNecessary(e);
                }
                throw AccountMchExceptions.ACQUIRE_LOCK_FAIL.newWithErrMsg("获取商户账务锁失败");
            }
        }
        ...
        try {
            //方法内部还有进一步做重复账务处理检测
            isSuccess = accountMchProcessBiz.executeSync(requestDto, groupMap);
        } catch (Throwable ex) {
            exProcessing = ex;
            logger.error("accountProcessNo={} 商户账务处理过程发生异常！", accountProcessNo, exProcessing);
        } finally {
            //释放锁
            try {
        ...
        try {
            processResultId = accountMchProcessBiz.afterSyncExecute(requestDto, processDtoList, isSuccess, exProcessing);
        } catch (Throwable exAfter) {
            logger.error("accountProcessNo={} 商户账务处理后发生异常", accountProcessNo, exAfter);
        }
        ...
        throwBizExceptionIfNecessary(exProcessing); // todo 有异常是否还需要发消息
        return isSuccess;
    }
    ...
    public boolean executeSyncForAsync(long accountProcessPendingId) {
        ...
        AccountMchProcessPending processPending = accountMchProcessPendingBiz.getAccountProcessPendingById(accountProcessPendingId, false);
        ...
        AccountMchRequestDto requestDto = JsonUtil.toBean(processPending.getRequestDto(), AccountMchRequestDto.class);
        List<AccountMchProcessDto> processDtoList = JsonUtil.toList(processPending.getProcessDtoList(), AccountMchProcessDto.class);
    ...
    public boolean executeAsync(AccountMchRequestDto requestDto, AccountMchProcessDto processDto) {
        List<AccountMchProcessDto> processDtoList = new ArrayList<>(1);
    ...
    public boolean executeAsync(AccountMchRequestDto requestDto, List<AccountMchProcessDto> processDtoList) {
        ...
        //2.执行异步账务处理
        Long processPendingId = accountMchProcessBiz.executeAsync(requestDto, processDtoList);
    ...
    public void commitDebitSync(List<String[]> merchantNoAndTrxNoList) {
        ...
        accountMchProcessBiz.commitDebitSync(merchantNoAndTrxNoList);
    }
     ...
     * @param processDtoList .
     */
    private void validAccountProcessParam(AccountMchRequestDto requestDto, List<AccountMchProcessDto> processDtoList) {
        if (requestDto == null) {
        ...
        } else if (processDtoList.stream().map(AccountMchProcessDto::getMerchantNo).collect(Collectors.toSet()).size() > MAX_USER_COUNT_PER_PROCESS) {
            throw CommonExceptions.PARAM_INVALID.newWithErrMsg("processDtoList中MerchantNo的个数不能超过" + MAX_USER_COUNT_PER_PROCESS);
        }

        for (AccountMchProcessDto dto : processDtoList) {
            if (dto.getTrxTime() == null) {
                ...
                throw CommonExceptions.PARAM_INVALID.newWithErrMsg("未识别的processType" + dto.getProcessType());
                    ...
                    || Arrays.stream(AccountMchAmountTypeEnum.values()).noneMatch(p -> p.getValue() == dto.getAmountType())) {
    ...
    private void notifyDoAccountProcess(String accountProcessNo, long processPendingId) {
        ...
        try {
            ...
            msgDto.setTopic(AccountMsgDest.TOPIC_ACCOUNT_MCH_PROCESS);
            msgDto.setTags(AccountMsgDest.TAG_ACCOUNT_MCH_PROCESS_ASYNC);
            ...
            boolean isSuccess = rmqSender.sendOne(msgDto);  //todo 如果丢失也可以接收，应该使用sendOneWay
    ...
    private void notifyDoAccountProcessCallBack(final String accountProcessNo, final Long processResultId) {
        ...
        try {
            MsgDto msgDto = new MsgDto();
            msgDto.setTopic(AccountMsgDest.TOPIC_ACCOUNT_MCH_PROCESS);
            msgDto.setTags(AccountMsgDest.TAG_ACCOUNT_MCH_CALLBACK);
            msgDto.setTrxNo(accountProcessNo);
