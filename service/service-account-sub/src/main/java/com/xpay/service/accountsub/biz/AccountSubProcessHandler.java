 ...
 */
@Component
public class AccountSubProcessHandler {
    private Logger logger = LoggerFactory.getLogger(AccountSubProcessHandler.class);
    //每次账务处理允许的商户数
     ...
     * @param processDto .
     */
    public boolean executeSync(AccountSubRequestDto requestDto, AccountSubProcessDto processDto) throws BizException {
        List<AccountSubProcessDto> processVoList = new ArrayList<>(1);
     ...
     */
    public boolean executeSync(AccountSubRequestDto requestDto, List<AccountSubProcessDto> processDtoList) throws BizException {
        ...
        accountSubProcessBiz.findDebitDetailAndFilterRepeatVo(processDtoList);
        if (processDtoList.isEmpty()) {
            logger.info("accountProcessNo={} 全部均已经账务处理过，无需重复处理！", accountProcessNo);
            throw AccountSubExceptions.ACCOUNT_PROCESS_REPEAT.newWithErrMsg("请勿重复账务处理");
        }

        //3、按用户分组
        Map<String, List<AccountSubProcessDto>> groupMap = processDtoList.stream().collect(Collectors.groupingBy(AccountSubProcessDto::getMerchantNo));

        //4.账务处理前置处理
        try {
            accountSubProcessBiz.beforeSyncExecute(requestDto, processDtoList, false);
        } catch (Throwable e) {
        ...
        try {
            logger.info("accountProcessNo={} 子商户账户获取锁开始", accountProcessNo);
            lockList = redisClient.tryLock(LockNameUtil.getAccountSubLockName(groupMap.keySet()), DistLockConst.ACCOUNT_LOCK_WAIT_MILLS, DistLockConst.ACCOUNT_LOCK_EXPIRE_MILLS);
            logger.info("accountProcessNo={} 子商户账户获取锁结束 lockList.size={}", accountProcessNo, lockList.size());
        } catch (Throwable t) {
            logger.error("accountProcessNo={} groupMap.size={} 子商户账户获取锁出现异常", accountProcessNo, groupMap.size(), t);
        } finally {
            if (lockList == null || lockList.size() != groupMap.size()) {
                logger.error("accountProcessNo={} groupMap.size={} lockList.size={} 子商户账户获取锁失败", accountProcessNo, groupMap.size(), lockList == null ? 0 : lockList.size());
                try {
                    //回退到'待处理'
                    accountSubProcessBiz.beforeSyncExecute(requestDto, processDtoList, true);
                } catch (Throwable e) {
                    logger.error("accountProcessNo={} 子商户待账务处理回退到'待处理'时发生异常", accountProcessNo, e);
                    throwBizExceptionIfNecessary(e);
                }
                throw AccountSubExceptions.ACQUIRE_LOCK_FAIL.newWithErrMsg("获取子商户账务锁失败");
            }
        }
        ...
        try {
            //方法内部还有进一步做重复账务处理检测
            isSuccess = accountSubProcessBiz.executeSync(requestDto, groupMap);
        } catch (Throwable ex) {
            exProcessing = ex;
            logger.error("accountProcessNo={} 子商户账务处理过程发生异常！", accountProcessNo, exProcessing);
        } finally {
            //释放锁
            try {
        ...
        try {
            processResultId = accountSubProcessBiz.afterSyncExecute(requestDto, processDtoList, isSuccess, exProcessing);
        } catch (Throwable exAfter) {
            logger.error("accountProcessNo={} 子商户账务处理后发生异常", accountProcessNo, exAfter);
        }
    ...
    public boolean executeSyncForAsync(long accountProcessPendingId) {
        ...
        AccountSubProcessPending processPending = accountSubProcessPendingBiz.getAccountProcessPendingById(accountProcessPendingId, false);
        ...
        AccountSubRequestDto requestDto = JsonUtil.toBean(processPending.getRequestDto(), AccountSubRequestDto.class);
        List<AccountSubProcessDto> processDtoList = JsonUtil.toList(processPending.getProcessDtoList(), AccountSubProcessDto.class);
    ...
    public boolean executeAsync(AccountSubRequestDto requestDto, AccountSubProcessDto processDto) {
        List<AccountSubProcessDto> processDtoList = new ArrayList<>(1);
    ...
    public boolean executeAsync(AccountSubRequestDto requestDto, List<AccountSubProcessDto> processDtoList) {
        ...
        //2.执行异步账务处理
        Long processPendingId = accountSubProcessBiz.executeAsync(requestDto, processDtoList);
    ...
    public void commitDebitSync(List<String[]> merchantNoAndTrxNoList) {
        ...
        accountSubProcessBiz.commitDebitSync(merchantNoAndTrxNoList);
    }
     ...
     * @param processDtoList .
     */
    private void validAccountProcessParam(AccountSubRequestDto requestDto, List<AccountSubProcessDto> processDtoList) {
        if (requestDto == null) {
        ...
        } else if (processDtoList.stream().map(AccountSubProcessDto::getMerchantNo).collect(Collectors.toSet()).size() > MAX_USER_COUNT_PER_PROCESS) {
            throw CommonExceptions.PARAM_INVALID.newWithErrMsg("processDtoList中MerchantNo的个数不能超过" + MAX_USER_COUNT_PER_PROCESS);
        }

        for (AccountSubProcessDto dto : processDtoList) {
            if (dto.getTrxTime() == null) {
                ...
                throw CommonExceptions.PARAM_INVALID.newWithErrMsg("未识别的processType" + dto.getProcessType());
                    ...
                    || Arrays.stream(AccountSubAmountTypeEnum.values()).noneMatch(p -> p.getValue() == dto.getAmountType())) {
    ...
    private void notifyDoAccountProcess(String accountProcessNo, long processPendingId) {
        ...
        try {
            ...
            msgDto.setTopic(AccountMsgDest.TOPIC_ACCOUNT_SUB_PROCESS);
            msgDto.setTags(AccountMsgDest.TAG_ACCOUNT_SUB_PROCESS_ASYNC);
    ...
    private void notifyDoAccountProcessCallBack(final String accountProcessNo, final Long processResultId) {
        ...
        try {
            MsgDto msgDto = new MsgDto();
            msgDto.setTopic(AccountMsgDest.TOPIC_ACCOUNT_SUB_PROCESS);
            msgDto.setTags(AccountMsgDest.TAG_ACCOUNT_SUB_CALLBACK);
            msgDto.setTrxNo(accountProcessNo);
