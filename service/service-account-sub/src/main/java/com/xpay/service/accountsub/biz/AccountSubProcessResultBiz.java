...
import com.xpay.common.statics.dto.accountsub.AccountSubProcessResultDto;
...
import com.xpay.facade.accountsub.entity.AccountSubProcessResultHistory;
import com.xpay.service.accountsub.dao.AccountSubProcessResultDao;
import com.xpay.service.accountsub.dao.AccountSubProcessResultHistoryDao;
 ...
 */
@Component
public class AccountSubProcessResultBiz {
    ...
    @Autowired
    private AccountSubProcessResultHistoryDao accountSubProcessResultHistoryDao;

    public void add(AccountSubProcessResult accountSubProcessResult) {
        accountSubProcessResultDao.insert(accountSubProcessResult);
    }

    public PageResult<List<AccountSubProcessResult>> listByPage(Map<String, Object> paramMap, PageParam pageParam, boolean inHistory) {
        if (!inHistory) {
            return accountSubProcessResultDao.listPage(paramMap, pageParam);
        } else {
            ...
            PageResult pageResult = accountSubProcessResultHistoryDao.listPage(paramMap, pageParam);
            List<AccountSubProcessResult> collect = ((List<AccountSubProcessResultHistory>) pageResult.getData()).stream()
                    .map(AccountSubProcessResult::newFromHistory).collect(Collectors.toList());
            pageResult.setData(collect);
            return pageResult;
        }
    }

    public AccountSubProcessResult getAccountProcessResultById(Long id, boolean inHistory) {
        if (id == null) {
            return null;
        }
        if (!inHistory) {
            return accountSubProcessResultDao.getById(id);
        } else {
            return AccountSubProcessResult.newFromHistory(accountSubProcessResultHistoryDao.getById(id));
        }
    }
    ...
    public boolean auditProcessResult(Long id, Integer auditType) {
        ...
        AccountSubProcessResult result = getAccountProcessResultById(id, false);
        if (result == null) {
            return false;
        ...
        if (auditType == AccountResultAuditTypeEnum.AUDIT_TO_SUCCESS.getValue()) {
            ...
            accountSubProcessPendingBiz.updatePendingStatus(result.getAccountProcessNo(), AccountProcessPendingStageEnum.PENDING, AccountProcessPendingStageEnum.FINISHED);
        }
        try {
            accountSubProcessResultDao.update(result);
            return true;
    ...
    public boolean sendProcessResultCallbackMsg(Long id, boolean isOnlySendMsg, boolean inHistory) {
        ...
        AccountSubProcessResult result = getAccountProcessResultById(id, inHistory);
        if (inHistory) {
            isOnlySendMsg = true;
        }
        ...
        //封装回调参数
        final AccountSubRequestDto requestDto = JsonUtil.toBean(result.getRequestDto(), AccountSubRequestDto.class);
        final AccountSubProcessResultDto resultDto = new AccountSubProcessResultDto();
        ...
        if (isOnlySendMsg) {
            ...
            successUpdate = accountSubProcessResultDao.updateAccountProcessResultToSentById(id);
        }
    ...
    public List<Long> listPendingAccountProcessResultId(Date modifyDate, int number) {
        ...
        return accountSubProcessResultDao.listAccountProcessResultId(paramMap);
    }

    private void fillProcessResultVo(AccountSubProcessResultDto resultDto, AccountSubProcessResult result, AccountSubRequestDto requestDto) {
        ...
        List<AccountSubProcessDto> processVoList = JsonUtil.toList(result.getProcessDtoList(), AccountSubProcessDto.class);
        for (AccountSubProcessDto processDto : processVoList) {
    ...
    public List<Long> listNeedCallBackResultId(Date createTimeBegin, Long minId, int number) {
        ...
        return accountSubProcessResultDao.listNeedCallBackResultId(createTimeBegin, minId, number);
    }
}
