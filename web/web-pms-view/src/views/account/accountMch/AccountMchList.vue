<template>
  <div class="main-page" v-loading="loading">
    <div class="main-page-header">
      ...
      <div class="search-item">
        <label>商户类型</label>
        <el-select v-model="searchParam.merchantType" clearable>
          <el-option v-for="{code,desc} in $dict('MerchantTypeEnum')" :label="desc" :key="parseInt(code)" :value="parseInt(code)"/>
        </el-select>
      </div>
    ...
    <div class="main-page-content">
      <el-table :data="pageResult.data" border row-key="id">
        ...
        <el-table-column label="商户类型" :formatter="row=>$dictCode('MerchantTypeEnum',row.merchantType).desc"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refreshList() {
        this.loading = true
        request.post(`/accountMch/listAccountMch`, Object.assign(this.searchParam, {createTimeBegin: this.createTimeRange[0], createTimeEnd: this.createTimeRange[1]}),
