<template>
  <div class="main-page" v-loading="loading">
    ...
    <div class="main-page-content">
      <el-table :data="pageResult.data" border row-key="id">
        ...
        <el-table-column label="操作" v-slot="{row}">
          <el-button type="text" size="small" @click="viewAccountProcessPendingPage(row)" v-permission="'account:mch:processPending:view'">查看详情</el-button>
        </el-table-column>
      </el-table>
    </div>
    ...
    <AccountMchProcessPendingView ref="processPendingView"/>
  </div>
</template>

<script>
  import request from "@/utils/request";
  import AccountMchProcessPendingView from "./AccountMchProcessPendingView";
  import {formatMoney} from "@/utils/commonUtil"
  import dayjs from "dayjs"

  export default {
    name: "AccountMchProcessPendingList",
    components: {
      AccountMchProcessPendingView,
    },
    data() {
      return {
        searchParam: {},
    ...
    methods: {
      ...
      refreshList() {
        this.loading = true
        request
          .post(`accountMch/listProcessPending?inHistory=${this.inHistory}`,
      ...
      viewAccountProcessPendingPage(row) {
        request.post(`accountMch/viewProcessPending?inHistory=${row.inHistory}`, `id=${row.id}`)
          .then(({data}) => {
