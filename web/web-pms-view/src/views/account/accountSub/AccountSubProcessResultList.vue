<template>
  <div class="main-page" v-loading="loading">
      ...
      <el-table :data="pageResult.data" border row-key="id" @selection-change="val=>selectedRow=val">
        ...
        <el-table-column label="操作" v-slot="{row}" width="190px">
          <el-button type="text" size="small" @click="viewAccountProcessResultPage(row)"
                     v-permission="'account:sub:result:view'">查看详情
          </el-button>
          ...
          <el-button type="text" size="small" @click="resendProcessResultCallback(row)"
                     ...
                     v-permission="'account:sub:result:callback'">发送通知
          </el-button>

          <el-dropdown size="small" placement="right" trigger="click" @command="auditProcessResult" v-permission="'account:sub:result:audit'"
    ...
    <account-sub-process-result-view ref="processResultView"/>
  </div>
</template>

<script>
  import request from "@/utils/request";
  import AccountSubProcessResultView from './AccountSubProcessResultView'
  import {formatMoney} from "@/utils/commonUtil"
  import dayjs from "dayjs"

  export default {
    name: "AccountSubProcessResultList",
    components: {
      AccountSubProcessResultView
    },
    data() {
      return {
        searchParam: {},
    ...
    methods: {
      ...
      refreshList() {
        this.loading = true
        request
          .post(`accountSub/listProcessResult?inHistory=${this.inHistory}`,
      ...
      viewAccountProcessResultPage(row) {
        request.post(`accountSub/viewProcessResult?inHistory=${row.inHistory}`, `id=${row.id}`)
          .then(({data}) => {
      ...
      resendProcessResultCallback(row) {
        this.$confirm('确认发送账务处理结果通知吗?')
          .then(() => request.post(`accountSub/resendProcessResultCallback?inHistory=${row.inHistory}`, `id=${row.id}`))
          .then(({data}) => {
      ...
      auditProcessResult({rows, auditType}) {
        ...
        this.$confirm(`确定将${rows.length}条账务处理结果审核为:" <span style="color: red;">${this.$dictCode('AccountResultAuditTypeEnum', auditType).desc}</span>" 吗?`, {dangerouslyUseHTMLString: true})
          .then(() => request.post(`/accountSub/auditProcessResult?auditType=${auditType}`, rows.map(row => row.id)))
          .then(({data}) => {
            this.$message.success(data)
            this.refreshList()
          })
