<template>
  <div class="main-page" v-loading="loading">
    ...
    <div class="main-page-content">
      <el-table :data="pageResult.data" border row-key="id">
        ...
        <el-table-column label="操作" v-slot="{row}">
          <el-button type="text" size="small" @click="viewAccountProcessPendingPage(row)" v-permission="'account:transit:processPending:view'">查看详情</el-button>
        </el-table-column>
      </el-table>
    </div>
    ...
    <account-transit-process-pending-view ref="processPendingView"/>
  </div>
</template>

<script>
  import request from "@/utils/request";
  import AccountTransitProcessPendingView from "./AccountTransitProcessPendingView";
  import {formatMoney} from "@/utils/commonUtil"
  import dayjs from "dayjs"

  export default {
    name: "AccountTransitProcessPendingList",
    components: {
      AccountTransitProcessPendingView,
    },
    data() {
      return {
        searchParam: {},
    ...
    methods: {
      ...
      refreshList() {
        this.loading = true
        request
          .post(`accountTransit/listProcessPending?inHistory=${this.inHistory}`,
      ...
      viewAccountProcessPendingPage(row) {
        request.post(`accountTransit/viewProcessPending?inHistory=${row.inHistory}`, `id=${row.id}`)
          .then(({data}) => {
