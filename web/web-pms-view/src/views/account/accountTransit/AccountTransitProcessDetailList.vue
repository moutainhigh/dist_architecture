    ...
    <div class="main-page-content">
      <el-table :data="pageResult.data" border row-key="id">
        ...
        <el-table-column width="150px">
          <template v-slot:header="{}">商户编号<br/>父商户编号</template>
          <template v-slot="{row}">{{row.merchantNo}}<br/>{{row.parentMerchantNo}}</template>
        </el-table-column>
        ...
        <el-table-column width="120px">
          <template v-slot:header="{}">在途余额变动金额<br/>在途金额</template>
...
<script>
  ...
  export default {
    name: "AccountTransitProcessDetailList",
    data() {
      return {
        searchParam: {},
    ...
    methods: {
      ...
      refreshList() {
        this.loading = true
        request
          .post(`/accountTransit/listProcessDetail?inHistory=${this.inHistory}`,
