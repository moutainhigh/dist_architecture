...
import com.xpay.csource.common.IniFileReader;
import com.xpay.csource.common.MyException;
...
public class ClientGlobal {
    ...
    /**
     * modified by caimengfei
     * 设置完tracker_group后，需要启动无效tracker的监听任务
     * @param trackerAddresses
     * @throws IOException
     * @throws MyException
     */
    public static void initByTrackers(InetSocketAddress[] trackerAddresses) throws IOException, MyException {
        if (g_tracker_group != null) {
            g_tracker_group.stopMonitorThread();
        }
        g_tracker_group = new TrackerGroup(trackerAddresses);
        g_tracker_group.startMonitorThread();
    }
    ...
    public static void setG_tracker_group(TrackerGroup tracker_group) {
        synchronized (lock) {
            if (tracker_group != null) {
                tracker_group.stopMonitorThread();
            }
