 ...
 */
public interface AccountMchProcessFacade {
     ...
     *                      若判定为重复账务处理,会抛出com.xpay.common.statics.exception.AccountMchExceptions#ACCOUNT_PROCESS_REPEAT
     */
    boolean executeSync(AccountMchRequestDto requestDto, AccountMchProcessDto processDto) throws BizException;

    boolean executeSync(AccountMchRequestDto requestDto, List<AccountMchProcessDto> processDtoList) throws BizException;
     ...
     * <p>
     * 2.内部会把 merchantNo + trxNo + processType 作为唯一约束的key，如果重复提交，会抛出异常：com.xpay.common.statics.exception.AccountMchExceptions#ACCOUNT_PROCESS_PENDING_UNIQUE_KEY_REPEAT
     * 3.如果账务处理流水号重复，则会抛出com.xpay.common.statics.exception.AccountMchExceptions#ACCOUNT_PROCESS_PENDING_PROCESS_NO_REPEAT
     *
     * @param requestDto 账务处理请求DTO
     *                   accountProcessNo:账务处理流水号，如果使用自定义的流水号，需要确保长度在15~30之间且只能由字母或数字或下划线组成
     *                   callbackDestination:RocketMQ的topic、tags 是可以用英文的冒号分割，如：my-topic:tags_one,不需要回调时可传入null
     ...
     */
    boolean executeAsync(AccountMchRequestDto requestDto, AccountMchProcessDto processDto) throws BizException;

    boolean executeAsync(AccountMchRequestDto requestDto, List<AccountMchProcessDto> processDtoList) throws BizException;
